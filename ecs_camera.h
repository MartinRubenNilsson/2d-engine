...
namespace ecs {
	...
	// bounds:             The camera view will be confined to this box. If the width or height is smaller
	//                     than that of the view, the view will be centered on the bounds along that axis.
	// shake_amplitude:    The amplitude of the camera shake.
	// shake_frequency:    The frequency of the camera shake.
	// _shake_offset:      The current camera shake offset.
	...
	constexpr Vec2f DEFAULT_CAMERA_SIZE = { GAME_FRAMEBUFFER_WIDTH, GAME_FRAMEBUFFER_HEIGHT };

	struct Camera {
		Vec2f center;
		Vec2f size = DEFAULT_CAMERA_SIZE;
		Rect2f bounds = Rect2f::FULL;
	...
	Rect2f get_active_camera_view(); // Hard-cuts when activating new camera.
	Rect2f get_blended_camera_view(); // Smoothly transitions between cameras.
