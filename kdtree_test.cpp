...
namespace kdtree_test {
	...
	void add_debug_shapes_to_render_queue() {
		...
		for (size_t index = 0; index < _kdtree.size(); ++index) {
			shapes::draw_point_later(_kdtree[index], Color::WHITE);
			if (index != nearest_index) continue;
			shapes::draw_circle_later(_kdtree[index], 2.f, Color::GREEN);
			shapes::draw_circle_later(cursor_position, sqrt(nearest_distance_sq), Color::GREEN);
		}
	}
}
