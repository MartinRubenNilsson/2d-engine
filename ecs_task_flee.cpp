...
namespace ecs {
	struct FleeTask {
		entt::entity target = entt::null; // the entity to flee
		float speed = 0.f;
	...
	void flee(entt::entity entity, entt::entity target, float speed, float safety_radius) {
		...
		_registry.emplace_or_replace<FleeTask>(entity, target, speed, safety_radius);
	}
	...
	void _update_flee_tasks(float dt) {
		for (auto [entity, task, flee] : _registry.view<Task, FleeTask>().each()) {
			...
			if (flee.safety_radius <= 0.f ||
				!_registry.valid(flee.target) ||
				!_registry.all_of<b2BodyId>(flee.target)) {
			...
			const b2BodyId target_body = _registry.get<b2BodyId>(flee.target);
