#include "stdafx.h"
#include "ecs_tiled.h"
#include "tiled_types.h"

namespace ecs {
	TMap::TMap(const tiled::Map* map)
		: _map(map)
	{}

	Vector2f TMap::get_bottom_right() const {
		return { (float)_map->width  * _map->tile_width,
			     (float)_map->height * _map->tile_height };
	}

	TTile::TTile(const tiled::Tile* tile, const tiled::Tileset* tileset)
		: _tile(tile), _tileset(tileset)
	{}

	TObject::TObject(const tiled::Object* obj, const tiled::Map* map)
		: _obj(obj), _map(map)
	{}

	TMap TObject::get_map() const {
		return { _map };
	}

	entt::entity TObject::get_id() const {
		return (entt::entity)_obj->id;
	}

	TObjectType TObject::get_type() const {
		return (TObjectType)_obj->type;
	}

	Vector2f TObject::get_position() const {
		return { _obj->x, _obj->y };
	}

	Vector2f TObject::get_top_left() const {
		Vector2f p = get_position();
		if (_obj->type == tiled::ObjectType::Tile) {
			p.y -= _obj->height;
		}
		return p;
	}

	std::span<const Vector2f> TObject::get_points() const {
		return { (const Vector2f*)_obj->points.data(), _obj->points.size() };
	}

	template<tiled::PropertyType type>
	struct GetPropertyRetVal {
		using Type = std::variant_alternative_t<(size_t)type, tiled::PropertyValue>;
	};

	template<>
	struct GetPropertyRetVal<tiled::PropertyType::String> {
		using Type = std::string_view;
	};

	template<>
	struct GetPropertyRetVal<tiled::PropertyType::File> {
		using Type = std::string_view;
	};

	template<>
	struct GetPropertyRetVal<tiled::PropertyType::Class> {
		using Type = std::string_view;
	};

	template<tiled::PropertyType type>
	GetPropertyRetVal<type>::Type _get_property(const tiled::Object& obj, std::string_view name) {
		constexpr size_t index = (size_t)type;
		for (const tiled::Property& prop : obj.properties) {
			if (prop.value.index() != index) continue;
			if (prop.name != name) continue;
			return std::get<index>(prop.value);
		}
		return {};
	}

	std::string_view TObject::get_string(std::string_view name) const {
		return _get_property<tiled::PropertyType::String>(*_obj, name);
	}

	int TObject::get_int(std::string_view name) const {
		return _get_property<tiled::PropertyType::Int>(*_obj, name);
	}

	float TObject::get_float(std::string_view name) const {
		return _get_property<tiled::PropertyType::Float>(*_obj, name);
	}

	bool TObject::get_bool(std::string_view name) const {
		return _get_property<tiled::PropertyType::Bool>(*_obj, name);
	}

	Color TObject::get_color(std::string_view name) const {
		tiled::Color color = _get_property<tiled::PropertyType::Color>(*_obj, name);
		return { color.r, color.g, color.b, color.a };
	}

	std::string_view TObject::get_file(std::string_view name) const {
		return _get_property<tiled::PropertyType::File>(*_obj, name);
	}

	entt::entity TObject::get_object(std::string_view name) const {
		return (entt::entity)_get_property<tiled::PropertyType::Object>(*_obj, name);
	}

	extern entt::registry _registry;

	void emplace_tiled_tile(entt::entity entity, const TTile& tile) {
		_registry.emplace_or_replace<TTile>(entity, tile);
	}

	void emplace_tiled_object(entt::entity entity, const TObject& obj) {
		_registry.emplace_or_replace<TObject>(entity, obj);
	}

	void clear_all_tiled_tiles_and_objects() {
		_registry.clear<TTile>();
		_registry.clear<TObject>();
	}
}
